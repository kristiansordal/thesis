...
\section{\romeq{}}
The results in this section present the performance metrics of experiments ran on 1-8 nodes with a single-socketed \romeq{} chip. As the nodes are single socketed, only one set of experiments are provided, where one MPI rank is placed per node, each with 16 OpenMP threads, as the \romeq{} chip only has 16 physical cores.
\input{romemulti.tex}
